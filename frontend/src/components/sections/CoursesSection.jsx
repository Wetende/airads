  ...
  return (
    <Box component="section" sx={{ py: 10, bgcolor: 'background.paper' }}>
      <Container maxWidth="lg">
        ...
        {/* Schools Section */}
        <Box sx={{ mb: 6 }}>
          <Typography variant="h5" sx={{ fontWeight: 'bold', textAlign: 'center', mb: 4, color: '#1e40af' }}>
            Our Campuses
          </Typography>
            ...
            {filters.map((filter) => (
              <Chip
                ...
                sx={{
                  ...
                  fontSize: '0.9rem',
                  transition: 'all 0.2s',
                  bgcolor: activeFilter === filter ? '#1e40af' : 'grey.100',
                  color: activeFilter === filter ? 'white' : 'text.secondary',
                  boxShadow: activeFilter === filter ? 2 : 0,
                  '&:hover': {
                    bgcolor: activeFilter === filter ? '#1d4ed8' : '#dbeafe',
                    color: activeFilter === filter ? 'white' : '#1e40af',
                  }
                }}
              />
            ))}
              ...
              >
                <Box sx={{ position: 'relative' }}>
                  ...
                  <Box sx={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.7) 0%, transparent 60%)', display: 'flex', alignItems: 'flex-end', p: 2 }}>
                    <Chip size="small" label={course.level} sx={{ bgcolor: '#dc2626', color: 'white', fontWeight: 'bold', fontSize: '0.7rem' }} />
                  </Box>
                </Box>
                <CardContent sx={{ flexGrow: 1, p: 3 }}>
                  <Typography variant="h6" component="h3" sx={{ fontWeight: 'bold', color: '#1e40af', mb: 2, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                    {course.title}
                  </Typography>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 'auto' }}>
                    <Chip size="small" label={course.category} sx={{ bgcolor: '#eff6ff', color: '#2563eb', fontWeight: 'bold' }} />
                    <Button 
                      size="small" 
